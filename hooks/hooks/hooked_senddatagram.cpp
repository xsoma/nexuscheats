	...
	if (*(int*)((uintptr_t)ecx + 0x164) == *(int*)((uintptr_t)ecx + 0x16C))
	{
		...
		if (netchannel && correct != g_ctx.globals.data.end())
		{
			...
			if (g_cfg.misc.extended_backtracking) {
				// the target latency.
				float correct = max(0.f, (200.f / 1000.f) - g_ctx.globals.ping - util::get_interpolation());

				m_clientstate()->pNetChannel->m_nInSequenceNr += 2 * (64 - 3) - static_cast<uint32_t>((64 - 3) * correct);
			}
		}
	}
