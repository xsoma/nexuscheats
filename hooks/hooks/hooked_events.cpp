...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		player_t* hurt = (player_t*)(m_entitylist()->GetClientEntity(user_id));
		player_t* attack = (player_t*)(m_entitylist()->GetClientEntity(attacker_id));

		if (attack == g_ctx.local() && hurt != g_ctx.local())
		{
			const auto mat = hurt->get_bone_array_for_write();

			hooks::add_matrix(hurt, mat);

			g_ctx.globals.hitted_shots[hurt->EntIndex()]++;
		}
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (g_cfg.esp.hitsound && g_cfg.player.enable)
			{
				...
				case 6:
					m_surface()->PlaySound_(crypt_str("hit.wav"));
					break;

				case 7:
					m_surface()->PlaySound_(crypt_str("laser.wav"));
					break;

				case 8:
					m_surface()->PlaySound_(crypt_str("click.wav"));
					break;
				}
			}
	...
	else if (!strcmp(event_name, crypt_str("round_start")))
	{
		...
		for (int i = 0; i < 65; i++)
		{
			g_ctx.globals.fired_shots2[i] = 0;
			g_ctx.globals.hitted_shots[i] = 0;
