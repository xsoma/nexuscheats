...
IMaterial* CreateMaterial(bool lit, const std::string& material_data)
{
	...
	auto matname = std::to_string(created);
	++created;
...
void hooks::add_matrix(player_t* player, matrix3x4_t* bones)
{
	auto& info = skeleton_info.emplace_back();
	std::memcpy(info.bones, bones, player->m_CachedBoneData().Count() * sizeof(matrix3x4_t));

	info.time = m_globals()->m_realtime + 3.4;
	...
	if (hdr) {
		static int m_nSkin = util::find_in_datamap(player->GetPredDescMap(), "m_nSkin");
		static int m_nBody = util::find_in_datamap(player->GetPredDescMap(), "m_nBody");

		info.render_info.origin = player->GetAbsOrigin();
		info.render_info.angles = player->GetAbsAngles();
		...
		info.render_info.hitboxset = player->m_nHitboxSet();
		info.render_info.skin = (int)(uintptr_t(player) + m_nSkin);
		info.render_info.body = (int)(uintptr_t(player) + m_nBody);
		info.render_info.entity_index = player->EntIndex();
		info.render_info.instance = call_virtual<ModelInstanceHandle_t(__thiscall*)(void*) >(renderable, 30)(renderable);
		...
		math::AngleMatrix(QAngle(info.render_info.angles.x, info.render_info.angles.y, info.render_info.angles.z), info.cur_bones);
	}
}

void hooks::draw_chams_on_hit()
{
	if (!g_cfg.player.chams_on_hit) return;
	...
	auto material = materials[g_cfg.player.chams_hit_material];

	//Hitmatrix
	{

		auto it = skeleton_info.begin();
		while (it != skeleton_info.end()) {
			if (!it->state.m_pModelToWorld || !it->state.m_pRenderable || !it->state.m_pStudioHdr || !it->state.m_pStudioHWData ||
				!it->render_info.pRenderable || !it->render_info.pModelToWorld || !it->render_info.pModel) {
				++it;
				continue;
			}
			...
			auto alphaB = (float)g_cfg.player.type[TEAM].xqz_color.a() / 255.0f;

			float xqz_color[3] =
			{
				g_cfg.player.chams_hit_color[0] / 255.0f,
				g_cfg.player.chams_hit_color[1] / 255.0f,
				g_cfg.player.chams_hit_color[2] / 255.0f
			};

			m_renderview()->SetBlend(alphaB);
			...
			m_modelrender()->DrawModelExecute(ctx, it->state, it->render_info, it->bones);
...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	if (is_player)
	{
		if (!model_entity->is_alive() || model_entity == nullptr) return;
