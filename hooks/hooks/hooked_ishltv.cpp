...
bool __fastcall hooks::hooked_ishltv(void* ecx, void* edx) 
{
	...
	//pq a função de update é justamente pra trocar
	if (g_ctx.globals.updating_animation && m_clientstate()->iDeltaTick != -1)
		return true;// esse nao é o original, pode ser isso, talvez. Vc tirou isso daonde?, eu troquei o resolver.cpp/h e pq mudou isso junto? pq tava no yougamebosta, mas nao mudei nada no hook
