...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (gameoverlay_return_address != (uintptr_t)(_ReturnAddress()) && g_cfg.misc.anti_obs)
		return;
	...
	if (!in_game && m_engine()->IsInGame()) //-V807
	{
		...
		}//borakkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk
	...
	if (reload_fonts())
	{
		...
		fonts[ESP] = create_font(crypt_str("Verdana"), 13,FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[NAME] = create_font(crypt_str("Verdana"), 14,FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[SUBTABWEAPONS] = create_font(crypt_str("undefeated"), 14, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		...
		fonts[DAMAGE_MARKER] = create_font(crypt_str("Verdana"), 20, FW_HEAVY, FONTFLAG_OUTLINE);
		fonts[VELOCITY] = create_font(crypt_str("Verdana"), 20, FW_HEAVY, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
		fonts[Nexus] = create_font(crypt_str("Nexus Regular"), 10, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			if (g_cfg.player.enable && g_cfg.misc.newscope &&  g_ctx.globals.scoped && weapon->is_sniper())
			{
				static int w, h;
				...
				auto color = Color(g_cfg.esp.scopecolor.r(), g_cfg.esp.scopecolor.g(), g_cfg.esp.scopecolor.b(), 255);
				auto end_color = Color(g_cfg.esp.scopecolor.r(), g_cfg.esp.scopecolor.g(), g_cfg.esp.scopecolor.b(), 0);
			...
			//walk_bot::get().WalkBot_painttraverse();
		...
		misc::get().watermark();
		misc::get().keybinds();//sla eu fechei
		
		
		worldesp::get().Wireframe();
		worldesp::get().RenderAsus();
		worldesp::get().no3dcu();
		otheresp::get().draw_velocity();
		worldesp::get().noeffects();
		worldesp::get().NoShadows();
