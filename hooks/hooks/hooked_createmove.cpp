...
struct dickk
{
	void MinWalk(CUserCmd* get_cmd, float get_speed)
	{
		...
		float min_speed = (float)(FastSqrt222(square(get_cmd->m_forwardmove) + square(get_cmd->m_sidemove)));
		if (min_speed <= 0.f)
			return;

		if (get_cmd->m_buttons & IN_DUCK)
		...
		get_cmd->m_forwardmove *= kys;
		get_cmd->m_sidemove *= kys;
		//get_cmd->m_upmove *= kys;
	}
}lowwalk;
...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	if(key_binds::get().is_enabled(21))
		lowwalk.MinWalk(m_pcmd, g_cfg.ragebot.slow_walk_velocity);
	//slowwalk::get().create_move(m_pcmd, g_cfg.ragebot.slow_walk_velocity);

	//walk_bot::get().MakeFebsMove(m_pcmd, g_ctx.local());
	...
	if (g_cfg.player.want_key && key_binds::get().is_enabled(25)) g_cfg.player.enable = true;
	else if(g_cfg.player.want_key && !key_binds::get().is_enabled(25)) g_cfg.player.enable = false;
