...
public:
	...
	void save_skin(std::string config);
	void reset_config(std::string config);
	void load(std::string config, bool load_script_items);
	void load_skin(std::string config);
...
struct Config 
{
	...
	struct Ragebot_t
	{
		...
		struct weapon
		{
			...
			std::vector <int> multi_hitboxes;
			std::vector <int> headaimcond; // esse eh pra prefer head (inutil)
			std::vector <int> headaimonlycond; // esse eh pra force head :)
			bool ignore_limbs_when_moving;
	...
	struct Player_t 
	{
		...
		bool chams_on_hit;
		int chams_hit_material;
		Color chams_hit_color;
	...
	struct Visuals_t
	{
		...
		bool molotov_shit;
		Color molotov_shit_team;
		Color molotov_shit_enemy;
		...
		int fog_distance;
		int world_transparency;
		int prop_transparency;
		Color autopeek_color;
		int fog_density;
		Color fog_color;
		std::vector <int> weapon;
		Color box_color;
	...
	struct Misc_t
	{
		...
		bool clicked_load = false;
		bool break_prediction = false;
	} misc;
	...
	struct Menu_t 
	{
		Color menu_theme = Color(117, 66, 245);
		bool watermark = true;
