...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
		...
		ImColor col = ImLerp(ImVec4{ 140 / 255.f, 140 / 255.f, 140 / 255.f, 1.f }, ImVec4{ 9 / 255.f, 169 / 255.f, 232 / 255.f, 1.f }, 1.f);
		//Fundo do selectable
		if (flags & ImGuiWindowFlags_Popup || flags & ImGuiWindowFlags_Tooltip) {
			ImU32 bg_col = ImColor(32, 32, 38);

			window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size, bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);

			window->DrawList->AddRect(ImGui::GetWindowPos(), ImGui::GetWindowPos() + window->Size, ImColor(59, 59, 59, int(1.f * 140)), 0);
			window->DrawList->AddRect(ImGui::GetWindowPos() + ImVec2(3, 3), ImGui::GetWindowPos() + window->Size - ImVec2(3, 3), ImColor(2, 2, 2, int(1.f * 170)), 0);
			//window->DrawList->AddRect(window->Pos, window->Pos + window->Size, col);
		}
