...
bool ImGui::ContentTab(const char* label, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
...
bool ImGui::MainTab(const char* label, ImFont* icon_font, const char* icon, bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImVec4 icon_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, g_cfg.menu.menu_theme.a() / 255.f);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
   ...
   // ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
	...
	window->DrawList->AddRect(ImVec2(bb.Min.x + 1, bb.Min.y + 1), ImVec2(bb.Max.x + -1, bb.Max.y + -1), ImColor(17, 17, 22, 0), 1, 15, 1.000000);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);

    RenderFrame(bb.Min, bb.Max, col, true, 2.f * c_menu::get().dpi_scale);

    window->DrawList->PushClipRect(bb.Min, bb.Max, false);
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, it_circle->second.alpha)), 60);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, it_filled->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item)
{
    ...
    const ImVec4 hover_act = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
    ...
    const ImVec4 arrow_act = ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    const ImVec4 hover_act = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, g_cfg.menu.menu_theme.a() / 255.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y - style.FramePadding.y - 5.5f), window->DC.CursorPos + ImVec2(w + 30.f, label_size.y * 1 + style.FramePadding.y / 2.0f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(0.f, label_size.y > 0.0f ? style.FramePadding.y + label_size.y / 5.f : 0.f));
	...
	//GetColorU32(ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg)
	//GetColorU32(ImGuiCol_FrameBgHovered);
	ImColor col = ImLerp(ImVec4{ 140 / 255.f, 140 / 255.f, 140 / 255.f, 1.f }, ImVec4{ g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, g_cfg.menu.menu_theme.a() / 255.f }, 1.f);
	...
	window->DrawList->AddRect(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x + -1, frame_bb.Max.y + -1), ImColor(17, 17, 22, 0), 1, 15, 1.000000);

	//window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), 2.f);
	//window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), 2.f, 15.f, 1.0f);
	...
	if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1.5f, 1.5f), grab_bb.Max + ImVec2(0.2f, 0.2f), GetColorU32(ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f)), 1.f);

	//if (label_size.x > 0.0f)
	  // RenderText(grab_bb.Max + ImVec2(0.2f, 0.2f), label);

	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
	auto Text_Size = CalcTextSize(value_buf, value_buf_end);

	if (hovered) {

		//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1.5f, 1.5f), grab_bb.Max + ImVec2(0.2f, 0.2f), GetColorU32(ImVec4(0, 0, 0, 1.f)), 15.f);
		ImVec4 g = ImVec4(g_cfg.menu.menu_theme.r() / 255.f, g_cfg.menu.menu_theme.g() / 255.f, g_cfg.menu.menu_theme.b() / 255.f, 1.f);
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(g), 2.f, 15, 1.0f);

		//if (grab_bb.Max.x > grab_bb.Min.x)


		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(229 / 255.f, 229 / 255.f, 229 / 255.f, 1.f));
		//ImGui::RenderText(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x, frame_bb.Min.y + style.FramePadding.y + -19.f), value_buf);
		ImGui::RenderText(grab_bb.Max + ImVec2(4.2f, -9.f), value_buf);
		ImGui::PopStyleColor();
	}

	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - label_size.y - 5), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = pGui::BeginChildD(label, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding, false);
        //PopStyleVar(3);
        //PopStyleColor();
        if (!child_visible)
        {
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts)
{
	...
	flags |= ImGuiColorEditFlags_NoInputs;/* flags |= ImGuiColorEditFlags_PickerHueWheel; */ flags |= ImGuiColorEditFlags_NoSidePreview;
    ...
    const bool alpha = (flags & ImGuiColorEditFlags_NoAlpha);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	auto textcolor = ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255);
	//auto g = ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255.f);
	if (selected)
	{
		textcolor = ImColor(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), 255);
	}
	else
	{
		textcolor = ImColor(200, 200, 200, 255);
	}
	/*else
	{
