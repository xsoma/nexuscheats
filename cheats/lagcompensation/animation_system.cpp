...
bool Above58(player_t* player)
{
	return !player->GetAnimOverlay(6)->m_flPlaybackRate > 0.000000;
}
...
void lagcompensation::update_player_animations(player_t* e)
{
		...
		if (!origin_delta.IsZero() && TIME_TO_TICKS(time_difference) > 0)
		{
			...
			if (e->m_fFlags() & FL_ONGROUND && animlayers[11].m_flWeight > 0.0f && animlayers[11].m_flWeight < 1.0f && animlayers[11].m_flCycle > previous_record->layers[11].m_flCycle)
			{
				...
				if (weapon)
				{
					...
					auto weapon_info = m_weaponsystem()->GetWeaponData(e->m_hActiveWeapon()->m_iItemDefinitionIndex());
					...
					if (modifier > 0.0f && modifier < 1.0f)
						animation_speed = max_speed * (modifier + 0.85f);
				}
			}
	...
	if (!player_info.fakeplayer && g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum() && !g_cfg.legitbot.enabled) //-V807
	{
		...
		float angle = e->m_angEyeAngles().y;
		float Delta = Above58(e) ? 90.f : HIGHDELTA ? e->get_max_desync_delta() : 35;
		switch (record->side)
		{
		case RESOLVER_ORIGINAL:
			...
			break;
		case RESOLVER_ZERO:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(angle);
			break;
		case RESOLVER_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(angle + Delta);
			break;
		case RESOLVER_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(angle - Delta);
			break;
		case RESOLVER_LOW_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(angle + 35.0f);
			break;
