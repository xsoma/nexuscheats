...
void lagcompensation::fsn(ClientFrameStage_t stage)
{
	...
	for (auto i = 0; i < m_globals()->m_maxclients; i++) //-V807
	{
		...
		if (e == nullptr || g_ctx.local() == nullptr || e == NULL || g_ctx.local() == NULL || !g_ctx.local()->is_alive() || !e->is_alive()) continue;
...
{
	if (e == nullptr || g_ctx.local() == nullptr || e == NULL || g_ctx.local() == NULL || !e->is_alive() || !g_ctx.local()->is_alive()) return;
...
#endif
	{
		...
		static float ang = 0;
		if (e->m_vecVelocity().Length2D() >= 50)
			ang = 29.f;
		else
			ang = 90.f;
		...
		case RESOLVER_ORIGINAL:
			...
			break;
		case LEGEND_1:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + ang);
			break;
		case LEGEND_2:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - ang);
			break;
