...
void eventlogs::paint_traverse()
{
	...
	for (size_t i = 0; i < logs.size(); i++)
	{
		...
		if (util::epoch_time() - log.log_time > 4600 && log.x >= 6.f) 
		{
		...
		auto log_size_x = render::get().text_width(fonts[LOGS], log.message.c_str());
		auto log_size_y = render::get().text_height(fonts[LOGS], log.message.c_str());
		
		render::get().rect_filled(log.x - 2, last_y + log.y - logs_size_inverted * 14, log_size_x + 3, log_size_y + 1, Color(29, 28, 32, 200));
		render::get().rect(log.x - 2, last_y + log.y - logs_size_inverted * 14, log_size_x + 3, log_size_y + 1, Color(56, 54, 59, 0));// n sei
		render::get().rect(log.x + 3, last_y + log.y - logs_size_inverted * 14 + 1, log_size_x - 4, log_size_y - 2, Color(17, 17, 22, 0));
		render::get().rect(log.x - 3, last_y + log.y - logs_size_inverted * 14 - 1, log_size_x + 4, log_size_y + 2, Color(17, 17, 22, 0));
...
void eventlogs::events(IGameEvent* event) 
{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && userid_id != m_engine()->GetLocalPlayer()) 
		{
			...
			ss << "Hitted: " << userid_info.szName << ". Hitbox: " << get_hitgroup_name(event->GetInt(crypt_str("hitgroup"))) << ". Damage: " << event->GetInt(crypt_str("dmg_health"));
		...
		else if (userid_id == m_engine()->GetLocalPlayer() && attacker_id != m_engine()->GetLocalPlayer()) 
		{
			...
			ss << "Damaged By: " << attacker_info.szName << ". Damage: " << event->GetInt(crypt_str("dmg_health")) << ". Hitbox: " << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_ITEM_PURCHASES] && !strcmp(event->GetName(), crypt_str("item_purchase")))
	{
		...
		std::stringstream ss;
		ss << "player: " << userid_info.szName << crypt_str(". purchased: ") << weapon;
...
{
	logs.emplace_front(loginfo_t(util::epoch_time(), "(Nexus) " + text, g_cfg.misc.log_color));
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ ASYMEL BETA ] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("(Nexus) "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ ASYMEL ALPHA ] ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0CASYMEL BETA \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("(Nexus) ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[ \x0C^ASYMEL ALPHA \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}