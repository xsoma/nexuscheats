...
void key_binds::update_key_binds()
{
	...
	update_key_bind(&g_cfg.player.enable_key, 25);
	//update_key_bind(&g_cfg.ragebot.weapon->damage_override_key, 26);
}
...
bool key_binds::is_enabled(int id)
{
	switch (id)
	{
	case 2:
	...
	case 25:
		return playeresp::get().esp_key_enabled;
	default:
		return keys[id];
	}
	//funcionou :D
