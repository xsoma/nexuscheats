...
inline float FastSqrt2222(float x)
{
...
struct dickk
{
	void MinWalk(CUserCmd* get_cmd, float get_speed)
	{
		...
		float min_speed = (float)(FastSqrt2222(square2(get_cmd->m_forwardmove) + square2(get_cmd->m_sidemove)));
		if (min_speed <= 0.f)
			return;
...
bool aim::automatic_stop(CUserCmd* cmd)
{
	...
	else
	{
		...
		cmd->m_forwardmove = negative_forward_direction.x * 0.7;
		cmd->m_sidemove = negative_side_direction.y * 0.7;
	}
...
//#endif
	...
	if (cmd->m_buttons |= IN_ATTACK && (shot->shot_info.hitchance > 0 || final_hitchance > 0))
		g_ctx.globals.fired_shots2[last_target_index]++;
