...
bool LabelClick(const char* label, bool* v, const char* unique_id)
{
	...
	if (*v)
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255 / 255.f, 0 / 255.f, 0 / 255.f, 1.f));
...
void lua_edit(std::string window_name)
{
	...
	auto get_dir = [&]() -> void
	{
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, path)))
			file_path = std::string(path) + crypt_str("\\KitsuneSquad\\Scripts\\");
...
namespace ImGui
{
	...
	bool Tab(const char* icon, const char* label, const char* desc, const ImVec2& size_arg, const bool selected)
	{
		...
		if (selected)
			window->DrawList->AddRectFilled({ bb.Max.x,bb.Max.y }, { bb.Max.x - 3,bb.Min.y }, ImColor(255 / 255.f, 0 / 255.f, 0 / 255.f, 255.f / 255.f));
		...
		window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(icon).y / 2), ImColor(255 / 255.f, 0 / 255.f, 0 / 255.f, 255.f / 255.f), icon);
		ImGui::PopFont();
...
// IMPORTANT: DO TAB SWITCHING BY LAST_TAB!!!!!
	...
	if (ImGui::Begin(crypt_str("legendware.pw v3"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
	{
		...
		ImGui::BeginChild("##Full", ImVec2{ 720, 560 }, false);
		{
			...
			ImGui::BeginChild("##Up", ImVec2{ 720, 50 }, false);
			{
				...
				//ImGui::PushFont(c_menu::get().futura_large); ImGui::TextColored(ImColor(255, 255, 255), "MEU BILAU"); ImGui::PopFont();
				...
				if (ImGui::Tab("1", "Rage", "HvH", ImVec2(85, 50), Active_Tab == 1 ? true : false))
					Active_Tab = 1;

				ImGui::SetCursorPos(ImVec2{ 335, 0 });
				if (ImGui::Tab("2", "Legit", "Aim", ImVec2{ 85, 50 }, Active_Tab == 2 ? true : false))
					Active_Tab = 2;
			...
			else if (Active_Tab == 2) // Legit | Ëĺăčň
			{
				...
				ImGui::BeginChild("##Main", ImVec2{ 160, 125 }, false, NoMove);
				{
					...
					ImGui::SetCursorPos(ImVec2{ 5, 65 });
					if (ImGui::SubTab("Trigger", ImVec2(150, 25), Active_Legit_Tab == 3 ? true : false))
						Active_Legit_Tab = 3;

					ImGui::SetCursorPos(ImVec2{ 5, 95 });
					if (ImGui::SubTab("Others", ImVec2(150, 25), Active_Legit_Tab == 4 ? true : false))
						Active_Legit_Tab = 4;
				}
				ImGui::EndChild();
			}
