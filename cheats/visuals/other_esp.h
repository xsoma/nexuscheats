...
struct m_indicator
{
	...
	m_indicator(int text, Color color) :
		m_text(std::to_string(text)), m_color(color)
	{

	}

	m_indicator(std::string tet, int text, Color color) :
		m_text(tet + std::to_string(text)), m_color(color)
	{
...
public:
	...
	void molotov_shit(entity_t* ent);
