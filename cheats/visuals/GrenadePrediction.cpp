...
void GrenadePrediction::Paint()
{
	...
	if (type && path.size() > 1)
	{
		...
		if (weapon->m_iItemDefinitionIndex() == WEAPON_SMOKEGRENADE)
			render::get().Draw3DFilledCircle(endpos, 144, 40.f, g_cfg.esp.grenade_prediction_tracer_color);
		else if (weapon->m_iItemDefinitionIndex() == WEAPON_MOLOTOV || weapon->m_iItemDefinitionIndex() == WEAPON_INCGRENADE)
			render::get().Draw3DFilledCircle(endpos, 150, 40.f, g_cfg.esp.grenade_prediction_tracer_color);
	}
}
