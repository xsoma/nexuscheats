...
void playeresp::paint_traverse()
{
	...
	for (auto i = 1; i < m_globals()->m_maxclients; i++) //-V807
	{
		...
		if (!e->is_alive() || e == nullptr) continue;
...
void playeresp::draw_box(player_t* m_entity, const Box& box)
{
	...
	render::get().rect(box.x, box.y, box.w - 4, box.h, color);
	render::get().rect(box.x + 1, box.y + 1, box.w - 2, box.h - 2, outline_color);
}
...
void playeresp::draw_health(player_t* m_entity, const Box& box, const HPInfo& hpbox)
{
	...
	auto color = m_entity->IsDormant() ? Color(130, 130, 130) : Color(0, 200, 0);
	...
	//render::get().rect(box.x - 8, box.y, 4, box.h, back_color);
	...
	render::get().rect_filled(n_box.x, n_box.y - 1, n_box.w, n_box.h + 2, back_color);
	render::get().rect_filled(n_box.x, n_box.y + offset, n_box.w, bar_height, color);
	render::get().rect(n_box.x, n_box.y + offset, n_box.w, bar_height, color);

	auto height = n_box.h - hpbox.hp * n_box.h / 100;

	if (hpbox.hp_difference)
		render::get().rect_filled(n_box.x, n_box.y + height - hpbox.hp_difference * n_box.h / 100, n_box.w, hpbox.hp_difference * n_box.h / 100, color);
...
bool playeresp::draw_ammobar(player_t* m_entity, const Box& box)
{
	...
	render::get().rect_filled(n_box.x - 1, n_box.y - 1, n_box.w, n_box.h, inner_back_color);
	render::get().rect_filled(n_box.x, n_box.y, bar_width, n_box.h , color);
	render::get().rect(n_box.x, n_box.y, bar_width, n_box.h , color);
...
void playeresp::draw_flags(player_t* e, const Box& box)
{
	...
	/*if (g_cfg.player.type[type].flags[FLAGS_MONEY])
	{
		auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(0, 255, 0, 255);
		color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()]);

		if(lagcompensation::get().player_resolver[e->EntIndex()].shoot)
			render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "SHOOTING");
		_y += 12;
	}*/
	...
	if (g_cfg.player.type[type].flags[FLAGS_ARMOR])
	{
		...
		if (helmet)
		{
			render::get().text(fonts[Nexus], _x, _y, color, HFONT_CENTERED_NONE, "a");
			_y += 10;
		}

		if (kevlar)
		{
			render::get().text(fonts[Nexus], _x, _y, color, HFONT_CENTERED_NONE, "b");
			_y += 10;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_SCOPED])
	{
		...
		if (scoped)
		{
			...
			render::get().text(fonts[Nexus], _x, _y, color, HFONT_CENTERED_NONE, "i"); //ZOOM
			_y += 10;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_KIT] && e->m_bHasDefuser())
	{
		...
		render::get().text(fonts[Nexus], _x, _y, color, HFONT_CENTERED_NONE, "r"); //DEFUSE
		_y += 10;
	}
	...
	{
		auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(255, 4, 0);
		color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()]);

		render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "LETHAL");
		_y += 10;
	}

	if (g_cfg.player.type[type].flags[FLAGS_C4] && e->EntIndex() == g_ctx.globals.bomb_carrier)
	{
		auto color = e->IsDormant() ? Color(130, 130, 130, 130) : Color(255, 123, 0);
		color.SetAlpha(255.0f * esp_alpha_fade[e->EntIndex()]);

		render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "C4"); //c4
		_y += 10;
	}
