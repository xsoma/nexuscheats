...
void bullettracers::draw_beam(bool local_tracer, const Vector& src, const Vector& end, Color color)
{
	...
	switch (g_cfg.esp.tracer_sel)
	{
	case 0:
		path = crypt_str("sprites/purpleglow1.vmt");
		break;
	case 1:
		path = crypt_str("sprites/purplelaser1.vmt");
		break;
	case 2:
		path = crypt_str("sprites/smoke.vmt");
		break;
	case 3:
		path = crypt_str("sprites/xfireball3.vmt");
		break;
	}
	beam_info.m_pszModelName = path;
