...
{
	arc_poly(Vector(x, y, 0), radius, DEG2RAD(min_angle), DEG2RAD(max_angle), 84, 3.5, Color(255, 255, 255));
}
...
int render::text_height(vgui::HFont font, const char* msg, ...)
{
	if (!m_surface())
		return 0.0f;
...
void render::Draw3DCircle(const Vector& origin, float radius, float points, Color color)
{
...
void render::filled_rect_world(Vector center, Vector2D size, Color color, int angle) {
	...
	if (math::world_to_screen(c_top_left, top_left) &&
		...
		Vertex_t vertices[4];
		//static int tex = g_surface->create_new_texture_id(true);
		static int tex = surface->CreateNewTextureID(true);

		//g_surface->draw_set_texture(tex);
		//g_surface->draw_set_color(color);
	...
	//    g_surface->draw_textured_polygon(4, vertices, true);
		surface->DrawTexturedPolygon(4, vertices, true);
	}
}


void render::Draw3DFilledCircle(const Vector& origin, float radius, float points, Color color)
{
	...
	for (float a = 0; a < (M_PI * 2.0f); a += step)
	{
		...
		if (m_debugoverlay()->ScreenPosition(start, start2d) || m_debugoverlay()->ScreenPosition(end, end2d) || m_debugoverlay()->ScreenPosition(origin, center2d))
			return;

		std::vector< Vertex_t >points
		{
			Vertex_t{Vector2D(center2d.x, center2d.y)},
			Vertex_t{Vector2D(start2d.x, start2d.y)},
			Vertex_t{Vector2D(end2d.x, end2d.y)}
		};

		//RenderTriangle(ImVec2(start2d.x, start2d.y), ImVec2(center2d.x, center2d.y), ImVec2(end2d.x, end2d.y), Color(color.r(), color.g(), color.b(), color.a() / 2));
		triangle(Vector2D(start2d.x, start2d.y), Vector2D(center2d.x, center2d.y), Vector2D(end2d.x, end2d.y), Color(color.r(), color.g(), color.b(), color.a() / 2));
