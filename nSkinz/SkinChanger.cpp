...
static const char* player_model_index_t[] =
{
	...
	"models/player/custom_player/legacy/tm_professional_varf.mdl", //proffesional
...
static const char* player_model_index_ct[] =
{
	...
	"models/player/custom_player/legacy/ctm_swat_variantf.mdl", //1st Lieutenant Farlow
	"models/player/custom_player/legacy/ctm_swat_variantg.mdl", //John 'Van Healen' Kask
	"models/player/custom_player/legacy/ctm_swat_varianth.mdl", //Bio-Haz Specialist
	"models/player/custom_player/legacy/ctm_swat_varianti.mdl", //Sergeant Bombson
	"models/player/custom_player/legacy/ctm_swat_variantj.mdl", //Chem-Haz Specialist
	"models/player/custom_player/legacy/ctm_swat_variante.mdl" //Cmdr. Mae 'Dead Cold' Jamison
};
...
{
	if (g_ctx.local() == NULL || g_ctx.local() == nullptr || local == NULL || local == nullptr) return;
...
void SkinChanger::run(ClientFrameStage_t stage) noexcept
{
	...
	if (!g_ctx.local() || g_ctx.local() == NULL || g_ctx.local() == nullptr)
		return;
...
{
	if (!g_ctx.local()->is_alive() || g_ctx.local() == NULL || g_ctx.local() == nullptr)
		return;
